#resume-item(
  [{% if entry.company %}{{ entry.company }}{% if entry.location %}, {{ entry.location }}{% endif %}{% elif entry.location %}{{ entry.location }}{% endif %}],
  ...
  {% if entry.position %}[{{ entry.position }}]{% else %}none{% endif %},
  [
  {% if entry.summary %}
    {{ entry.summary }}
  {% endif %}
