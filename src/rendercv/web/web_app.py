...
app = FastAPI(
    title="RenderCV Web Editor",
    description="Live YAML Editor & PDF Generator for RenderCV",
)
...
def get_template(theme_name: str) -> dict[str, str]:
    ...
    try:
        yaml_content = create_sample_yaml_input_file(
            file_path=None, name="John Doe", theme=theme_name
        )
...
def switch_theme(request: SwitchThemeRequest):
    ...
    try:
        ...
        new_theme_yaml = create_sample_yaml_input_file(
            file_path=None, name="John Doe", theme=request.theme
        )
...
def render_pdf(request: RenderRequest):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            if (
                "cv" in input_dict
                and "sections" in input_dict["cv"]
                and input_dict["cv"]["sections"]
            ):
            ...
            model = build_rendercv_model_from_commented_map(
                input_dict, tmp_path, overlay_sources
            )
            ...
            if not typst_path:
                raise HTTPException(
                    status_code=400, detail="Failed to generate Typst code."
                )
            ...
            if not pdf_path or not pdf_path.exists():
                raise HTTPException(
                    status_code=500, detail="PDF compilation failed to produce output."
                )
            ...
            return JSONResponse(
                status_code=400,
                content={"type": "user_error", "detail": str(e.message or e)},
            )
        except Exception as e:
            return JSONResponse(
                status_code=500,
